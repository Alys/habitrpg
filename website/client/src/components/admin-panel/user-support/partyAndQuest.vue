    ...
    <div v-if="expand">
      ...
      >
        <p v-if="partyNotExistError">
          ...
          Ask a database admin to delete their Party ID ({{ userPartyData._id }}).
        </p>
        <p
          v-if="questErrors"
          v-html="questErrors"
...
function determineQuestStatus (self) {
  ...
  if (self.userHasParty && groupQuestData) {
    ...
    if (groupQuestData.key) {
      ...
      if (!groupQuestData.members) {
        ...
        if (questIsActive) {
          self.questErrors = 'ERROR: Quest is running but user\'s invitation is still pending in quest object.';
        } else {
  ...
  if (questExists) {
    questStatus += '<p>';
    const countOfQuestPartipants = Object.keys(groupQuestData.members).length;
    if (countOfQuestPartipants) {
      ...
      self.questErrors += 'ERROR: Quest has no participants.<br>';
    }
    questStatus += '</p>';
  }
  ...
  if (inviteStatusForUser === 'pending' || inviteStatusForUser === 'accepted') {
    if (!self.userPartyData.quest.key) {
      ...
      self.questErrors += 'ERROR: User has accepted quest invitation but the "key" in '
        + `their account (${self.userPartyData.quest.key}) is different than the quest's `
        + `"key" (${groupQuestData.key}).<br>`;
    }
  } else if (self.userPartyData.quest.key) {
    self.questErrors += 'ERROR: User has a "key" for the quest but probably should not '
  ...
  if (questExists) {
    const questContent = this.content.quests[groupQuestData.key];
    if (questContent) {
...
function resetData (self) {
  self.questStatus = '';
  // XXX do I need to reset self.partyNotExistError ?
  self.questErrors = '';
  ...
  if (self.partyNotExistError) {
    ...
    self.userIsPartyLeader = self.groupPartyData.leader === self.heroId;
...
export default {
  ...
  data () {
    return {
      ...
      questStatus: '',
      // XXX need this? partyNotExistError: '',
      questErrors: '',
